...
function inferMessageType(url: string | null | undefined) {
  if (!url) return "text";
  const ext = url.split(".").pop()?.toLowerCase();
  switch (ext) {
    case "jpg":
...
// Gerar conteúdo placeholder para mídia
function generateContentForMedia(type: string, name?: string | null) {
  switch (type) {
    case "image":
      return `📷 Imagem${name ? `: ${name}` : ""}`;
    case "video":
      return `🎥 Vídeo${name ? `: ${name}` : ""}`;
    case "audio":
      return `🎵 Áudio${name ? `: ${name}` : ""}`;
    case "document":
...
/** Normaliza e valida a URL do webhook do n8n (prod), evitando erros comuns */
function normalizeN8nUrl(raw: string) {
  if (!raw) throw new Error("Empty N8N URL");
  let input = raw.trim();

  // Adiciona https:// se vier sem esquema
  ...
  // Força produção: webhook-test -> webhook
  u.pathname = u.pathname.replace(/\/webhook-test\//, '/webhook/');

  // Corrige duplicação de /message/message
  u.pathname = u.pathname.replace(/\/message\/message\/?$/, '/message');

  // Remove barra final extra (exceto raiz)
  if (u.pathname !== '/' && u.pathname.endsWith('/')) {
    u.pathname = u.pathname.slice(0, -1);
  }

  // Garante que exista /webhook/ no caminho
  if (!/\/webhook(\/|$)/.test(u.pathname)) {
    const parts = u.pathname.split('/').filter(Boolean);
    const last = parts.pop() || 'message';
    const base = parts.join('/');
    u.pathname = `/${base ? base + '/' : ''}webhook/${last}`;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (base64Data && !fileUrl) {
      ...
      try {
        let actualBase64Data: string;
        let detectedMimeType = mimeType as string | null | undefined;
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from('whatsapp-media')
          .upload(storagePath, binaryData, { contentType: detectedMimeType, upsert: true });
    ...
    const senderType = payload.sender_type ?? (payload.messageTimestamp || payload.messageTime || payload.received_at || payload.from_contact ? "contact" : "agent");
    const messageStatus = payload.status ?? (senderType === "contact" ? "received" : "sent");
    const finalMessageType = messageTypeRaw || inferMessageType(fileUrl || "");
    const hasValidContent = !!(responseMessage || (fileUrl && finalMessageType !== "text") || base64Data);
    ...
    let workspaceId = directWorkspaceId; // Permitir override direto
    let finalConversationId = conversationId;
    let resolvedConnectionId = connectionId;
    let resolutionMethod = directWorkspaceId ? 'direct_override' : 'auto_resolve';
    console.log(`🔍 [${requestId}] Starting workspace resolution (${dataSource}) with data:`, {
      directWorkspaceId, conversationId, connectionId, evolutionInstance, instanceId, remoteJid, phoneNumber, resolutionMethod
    });

    if (!workspaceId && conversationId) {
      console.log(`🔍 [${requestId}] Resolving workspace from conversation_id: ${conversationId}`);
      const { data: conversation, error: convError } = await supabase
        .from('conversations').select('workspace_id, connection_id').eq('id', conversationId).maybeSingle();

      if (convError) {
        console.error(`❌ [${requestId}] Error querying conversation:`, convError);
        return new Response(JSON.stringify({
          code: 'DATABASE_ERROR', message: 'Error querying conversation', details: convError.message, requestId
        }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      if (!conversation) {
        console.error(`❌ [${requestId}] Conversation not found: ${conversationId}`);
        return new Response(JSON.stringify({
          code: 'CONVERSATION_NOT_FOUND', message: 'Specified conversation_id not found', requestId
      ...
      console.log(`🔍 [${requestId}] Resolving workspace from connection_id: ${connectionId}`);
      const { data: connection, error: connError } = await supabase
        .from('connections').select('workspace_id, instance_name').eq('id', connectionId).maybeSingle();

      if (connError) {
        console.error(`❌ [${requestId}] Error querying connection:`, connError);
        return new Response(JSON.stringify({
          code: 'DATABASE_ERROR', message: 'Error querying connection', details: connError.message, requestId
        }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      if (!connection) {
        console.error(`❌ [${requestId}] Connection not found: ${connectionId}`);
        return new Response(JSON.stringify({
          code: 'CONNECTION_NOT_FOUND', message: 'Specified connection_id not found', requestId
        }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      workspaceId = connection.workspace_id;
      resolutionMethod = 'connection_id';
      ...
      if (evolutionInstance) {
        const result = await supabase.from('connections')
          .select('id, workspace_id, instance_name').eq('instance_name', evolutionInstance).maybeSingle();
        connection = result.data;
        connError = result.error;
      }
      if (!connection && instanceId) {
        ...
        const result = await supabase.from('connections')
          .select('id, workspace_id, instance_name, metadata')
          .filter('metadata->remote_id', 'eq', instanceId).maybeSingle();
        connection = result.data;
        connError = result.error;
      }
      if (connError) {
        console.error(`❌ [${requestId}] Error querying instance:`, connError);
        return new Response(JSON.stringify({
          code: 'DATABASE_ERROR', message: 'Error querying evolution instance', details: connError.message, requestId
        }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      if (!connection) {
        console.error(`❌ [${requestId}] Instance not found: ${searchValue}`);
        return new Response(JSON.stringify({
          code: 'INSTANCE_NOT_FOUND',
          message: `Specified ${evolutionInstance ? 'evolution_instance' : 'instanceId'} not found`,
          requestId
      ...
      const sanitizedPhone = sanitizePhoneNumber(phoneNumber);
      const { data: existingContact, error: contactError } = await supabase
        .from('contacts').select('workspace_id').eq('phone', sanitizedPhone).limit(1).maybeSingle();
    ...
    // Se ainda não temos conversation_id, resolver via phoneNumber
    if (!finalConversationId && phoneNumber && workspaceId) {
      console.log(`🔍 [${requestId}] Creating/finding conversation for phone: ${phoneNumber} in workspace: ${workspaceId}`);
      const sanitizedPhone = sanitizePhoneNumber(phoneNumber);
      try {
        ...
        let { data: existingContact, error: findContactError } = await supabase
          .from('contacts').select('id, name').eq('phone', sanitizedPhone).eq('workspace_id', workspaceId).maybeSingle();

        if (findContactError) {
          console.error(`❌ [${requestId}] Error finding contact:`, findContactError);
          return new Response(JSON.stringify({
            code: 'DATABASE_ERROR', message: 'Error finding contact', details: findContactError.message, requestId
          }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        if (!existingContact) {
          console.log(`➕ [${requestId}] Creating new contact for phone: ${sanitizedPhone}`);
          ...
          if (createContactError) {
            console.error(`❌ [${requestId}] Error creating contact:`, createContactError);
            return new Response(JSON.stringify({
              code: 'DATABASE_ERROR', message: 'Error creating contact', details: createContactError.message, requestId
            }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
          }
          existingContact = newContact;
        }
        ...
        if (findConvError) {
          console.error(`❌ [${requestId}] Error finding conversation:`, findConvError);
          return new Response(JSON.stringify({
            code: 'DATABASE_ERROR', message: 'Error finding conversation', details: findConvError.message, requestId
          }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        if (!existingConv) {
          console.log(`➕ [${requestId}] Creating new conversation for contact: ${existingContact.id}`);
          ...
          if (createConvError) {
            console.error(`❌ [${requestId}] Error creating conversation:`, createConvError);
            return new Response(JSON.stringify({
              code: 'DATABASE_ERROR', message: 'Error creating conversation', details: createConvError.message, requestId
            }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
          }
          finalConversationId = newConv.id;
        } else {
        ...
        console.error(`❌ [${requestId}] Unexpected error during upsert:`, error);
        return new Response(JSON.stringify({
          code: 'UNEXPECTED_ERROR', message: 'Error during contact/conversation creation', details: String(error?.message ?? error), requestId
        }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    }
    ...
    if (hasValidContent) {
      ...
      if (externalId) messagePayload.external_id = externalId;

      console.log(`💾 [${requestId}] Inserting message into conversation: ${finalConversationId}`);
      const { data: insertedMessage, error: msgError } = await supabase
        .from("messages").insert(messagePayload).select().single();

      if (msgError) {
        console.error(`❌ [${requestId}] Failed to insert message:`, {
          error: msgError.message, code: msgError.code, hint: msgError.hint, details: msgError.details, payload: messagePayload
        });
        if (msgError.code === '23505' && msgError.message.includes('external_id')) {
          console.log(`ℹ️ [${requestId}] Duplicate message ignored (idempotent)`);
        } else {
          return new Response(JSON.stringify({
            ...
            message: 'Failed to insert message',
            details: msgError.message,
            hint: msgError.hint ?? msgError.details ?? null,
            requestId
          }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
      } else {
        ...
        if (senderType === "contact") {
          // @ts-ignore – supabase.raw para incremento
          conversationUpdate.unread_count = supabase.raw('unread_count + 1');
        } else {
          conversationUpdate.unread_count = 0;
        }
        const { error: updateError } = await supabase.from("conversations").update(conversationUpdate).eq("id", finalConversationId);
        if (updateError) console.error(`⚠️ [${requestId}] Error updating conversation:`, updateError);
    ...
    if (webhookError) {
      ...
      try {
        const normalized = normalizeN8nUrl(workspaceWebhookUrl);
        console.log(`📤 [${requestId}] N8N webhook (raw): ${workspaceWebhookUrl}`);
        console.log(`📤 [${requestId}] N8N webhook (normalized): ${normalized}`);
        workspaceWebhookUrl = normalized;

        const parsed = new URL(workspaceWebhookUrl);
        console.log(`📤 [${requestId}] Using workspace webhook: ${parsed.hostname}${parsed.pathname}`);
      } catch (e) {
        console.error(`❌ [${requestId}] Invalid N8N URL in DB:`, workspaceWebhookUrl, e);
        return new Response(JSON.stringify({
          code: 'INVALID_WEBHOOK_URL',
          message: 'Workspace webhook URL is invalid',
          requestId
        }), { status: 424, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
    }
    ...
    // HEAD opcional para diagnóstico
    try {
      const head = await fetch(workspaceWebhookUrl, { method: 'HEAD' });
      console.log(`🧭 [${requestId}] N8N HEAD status: ${head.status}`);
    } catch (e) {
      console.log(`🧭 [${requestId}] N8N HEAD error: ${String(e)}`);
    }
    ...
    try {
      ...
      if (!n8nResponse.ok) {
        ...
        return new Response(JSON.stringify({
          ...
          details: errorText,
          called_url: workspaceWebhookUrl,
          requestId
