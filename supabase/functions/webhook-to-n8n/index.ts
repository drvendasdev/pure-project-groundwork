...
serve(async (req) => {
  ...
  try {
    console.log('Configurando webhook para N8N...');
    ...
    const response = await fetch('https://evo.eventoempresalucrativa.com.br/webhook/set/Empresa-3', {
      ...
      body: JSON.stringify({
        url: "https://n8n-n8n-tezeus.fi1pd6.easypanel.host/webhook-test/message",
    ...
    return new Response(JSON.stringify({
      ...
      result: result,
      message: "Webhook configurado para N8N diretamente"
    }), { 
      headers: { 
        'Content-Type': 'application/json',
